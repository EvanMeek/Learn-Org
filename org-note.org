...
*** 代办事项基础
**** 基础的代办事项功能
     当标题的开头为"TODO"时，任何标题都会成为一个代办事项，例如:
     #+begin_src org
       ,* TODO 完成ORG-MODE笔记xxx部分
     #+end_src
     可以通过按键_C-c C-t_来对一个已存在标题标记不同的代办状态，默认情况下有三种代办状态:
     - TODO :: 代办事项
     - DONE :: 已完成的代办事项
     - unmarked :: 未标记为代办事项的标题，也就是普通的标题

     还可以通过agenda缓冲区的~t~命令按键来远程完成切换不同代办状态。
     
     下面是一些按键说明:

     - _S-RIGHT/LEFT_ :: 选择前或后一种代办状态，类似于循环切换代办状态.
     - _C-c / t_ :: 用[[*Sparse Trees 不知道咋翻译，称为稀疏树][Sparse Trees]]浏览代办事项。这将会折叠整个buffer，但是会显示所有的代办状态（除了DONE状态），以及标题的层级关系。
     - ~M-x org-agenda t~ :: 显示全局的代办事项，代办事项列表从所有的agenda文件中收集，然后在一个单独的缓冲区内显示。
     - _S-M-RET_ :: 新建一个代办事项
***** 其他
      也可以通过改变标签从而修改代办事项状态，具体见函数[[help:org-todo-trigger-tag-changes][org-todo-tigger-tag-changes]]的文档。
**** 多种工作流状态
     ...
     #+end_src
     以上代码让你在为一个标题提供代办事项状态时添加了一个键序列，你可以键入_C-c C-t_来查看效果。

     如果你想为单独的Org文件提供一个代办状态，并且为其添加键序列，可以使用如下配置:
     #+begin_src org
       ,#+TODO: TODO(t) | DONE(d)
     ...
     #+end_src
**** 进展日志
     记录进展日志以及改变代办状态的时间戳可以使用前缀调用命令_org-todo_。
     
     当你键入_C-u C-c C-t_将会提示你改变当前标题的代办状态，如果你将一个代办状态从TODO状态改为DONE状态的话，那么org就会为你创建一个时间戳，以记录你完成当前代办事项的时间。这个功能是基于一个特定的变量决定的：
     #+begin_src emacs-lisp
     ...
     #+end_src
**** 优先级
     当你大量使用Org模式后，你可能会有大量的代办事项条目，这个时候你可以为其设置优先级标记:
     #+begin_src org
       ,*** TODO [#A] 洗碗
     #+end_src
...
**** 任务细分
     我们可以将一个代办事项分为多个小的代办事项，并且可以用一些标记用于对小的代办事项做统计：
     #+begin_src org
       ,* TODO 每日任务 [33%]
       ,** TODO 写作业 [2/3]
       ,*** TODO 语文
       ...
       ,*** DONE 副菜
           ...
           - 鱼汤
     #+end_src
     代办事项的进度有两种呈现方式，一种是标记为~[/]~另一种是标记为~[%]~，如果它们的子项已完成，那么当前代办事项会自动切换状态为DONE。
     _C-c C-c_ 刷新进度的状态
**** 复选框
     还记得清单列表么，其中有一个复选框的样式，复选框不会被包含在全局的代办事项列表中，因此很适合用来将一个代办事项划分为多个步骤的操作：
     #+begin_src org
       ,* TODO 每日任务 [33%]
       ,** TODO 写作业 [3/3]
          - [X] 语文
